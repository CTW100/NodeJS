...
const transporter = nodemailer.createTransport(
  sendgridTransport({
    auth: {
      api_key:
        "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",
    },
  })
);
...
exports.postLogin = (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    return res.status(422).render("auth/login", {
      path: "/login",
      pageTitle: "Login",
      eerrorMessage: errors.array()[0].msg,
      oldInput: {
        email: email,
        password: password,
      },
    ...
    .then((user) => {
      if (!user) {
        return res.status(422).render("auth/login", {
          path: "/login",
          pageTitle: "Login",
          eerrorMessage: "Invalid email or password.",
          oldInput: {
            email: email,
            password: password,
          },
